...
//
//	Description: Source file for USB firmware. Includes top level isr with Setup,
//	             and Endpoint data handlers.  Also includes routine for USB 
//              suspend, reset, and procedural stall.
...
extern BYTE data Out_Packet[];
extern BYTE data In_Packet[];
extern BYTE data InPktRdy;
extern BYTE data OutPktRdy;
extern unsigned char data USBRXCtr;
...
POLL_READ_BYTE(EINCSR1, CntlReg); 		// Read contol register for EP 1
...
//EB			                            // Break Data into multiple packets if larger than Max Packet
...
//EB!!
			if(InPktRdy)	//If packet ready, write to FIFO, otherwise ZLpacket will be sent
			{            // Put new data on Fifo
    	  Fifo_Write(FIFO_EP1, EP1_PACKET_SIZE, (BYTE *)In_Packet);
				InPktRdy=0;											//Semaphore to polling loop
			}
...
//      if (Count != EP2_PACKET_SIZE)     // If host did not send correct packet size, flush buffer
      if (OutPktRdy || (Count != EP2_PACKET_SIZE))  // If not ready or not correct packet size, flush buffer
      {
      ...
      else                              // Otherwise get the data packet
      {
				...
				OutPktRdy=1;									//Semaphore to polling loop
				USBRXLED=LEDON;	// Turn on the USB LED to show activity
				USBRXCtr=10;		// Set counter for T2 ISR to turn off 10 x 0.1s later
      }
