...
//
// Description: This source file contains the subroutines used to handle 
//              incoming setup packets. These are called by Handle_Setup in 
//              USB_ISR.c and used for USB chapter 9 compliance.
...
extern BYTE code HidReportDesc[];
...
//            DataPtr = (BYTE*)&ZERO_PACKET;      // Otherwise send 0x00, indicating bus power and no
            DataPtr = (BYTE*)&ONES_PACKET;      // Otherwise send 0x00, indicating self-power and no
...
{ 
unsigned char code* CharDataPtr;

//	CharDataPtr = 0x7800;
//	FlashAddr = 0x7880;
	CharDataPtr = FlashAddr;


	if (Ep_Status[0] != EP_TX)
	{
	  DataPtr = (BYTE*) CharDataPtr;
		DataSize = 64;
	}
...
// HID specific, not implemented. Placeholder functions.
...
void Get_Idle(void) 
{
	// Get_Idle command not supported, so send a stall to the host
	Force_Stall();
}  

void Set_Idle(void)
{
	// Set_Idle command not supported, so send a stall to the host
	Force_Stall();
}  
